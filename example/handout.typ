...
#summary-block(
  notion: [
    - 剪切流动：速度梯度方向与流动方向垂直；拉伸流动：速度梯度方向与流动方向相同。
    - 剪切速率：单位时间内的剪切应变。
    - 剪切应力：单位面积上的剪切力。
    - 剪切粘度：剪切应力和剪切速率的比值，称为剪切粘度，也叫表观粘度，通常简称为粘度。
    - 零切粘度：剪切速率趋于零时的粘度。
...
$
  D_e=(k T) / zeta_"tot"=(k T) / (N zeta_0)
$

其中$zeta_"tot"$和$zeta_0$分别为整链和链段受溶剂摩擦阻力，$N$为一根分子链中的链段数目。
...
其中$L_"eq"$为缠结形成的管子的长度。$L_"eq" =$长度$times$段数

$
  L_"eq" =sqrt(N_e) times N / N_e∝N
$

故

$
  lambda = (L_"eq")^2 / D ∝ N^2 / (1 slash N)∝ N^3
$

而又因为模量取决于缠结密度，$G=(rho N_A k T) / M_e$，与总分子量无关。所以$eta_0 =lambda G∝ N^3$。
...
均方回转半径$"<"R^2">"=2 n D t ∝N$。其中，$D∝N^(-2),t∝N^(3)$

可得松弛时间（Rouse时间）

$
  lambda≈R^2 / D_R≈R^2 / (k T slash N zeta)=zeta / (k T) N R^2∝N^2
$

Rouse模型中弹性主要来源于单链的熵弹性，$G∝N^(-1)$。故
...
+ 若支链虽长，但*其长度还不足以使支链本身发生缠结*，这时体系自由体积体积增大。与分子量相当的线型聚合物比，支化聚合物的粘度要低些。

  注：此处粘度比较主要为零切粘度，和分子本身形状关系大。

+ 若支链相当长，*长支链本身发生缠结*，长支化聚合物的非牛顿性较强，在高剪切速率下，支化聚合物比分子量相当的线型聚合物的粘度低。

  注：此处粘度比较是高剪切速率下的粘度比较，注意与第二点做区分。
+ 超支化大分子的粘度在分子量超过一定数值后，反而随分子量提高而下降。
+ *长支链使熔体强度增加，有利于吹膜、发泡。*（支链对加工性能的影响，与前文“拉伸硬化”内容可相互结合。）
...
$
  [eta] prop (M^(1.8)) / M = M^0.8
$

- $theta$溶液：$v=0.5$,

$
  [eta] prop (M^(1.5)) / M=M^0.5
$
...
假设一块橡胶在室温下受外力$f$从起始长度$l_0$拉伸到$l$。根据热力学第一定律（能量守衡与转化定律），体系内能的变化等于吸收的热量减去对外做的功，有$dif u=dif Q-dif W$；根据热力学第二定律，假设过程是可逆的，有$dif Q=T dif S$。又体系对外作功为拉伸过程中体积变化$p dif V$和形状变化$-f dif l$之和，故有$dif W=p dif V-f dif l$。假设在拉伸过程中体积不变，$dif V=0$，$dif W=-f dif l$。即$dif u=dif Q+f dif l=T dif S+f dif l$。
...
这说明橡胶的张力是由形变时内能的变化和熵的变化引起的。令$f_u=((partial u) / (partial l))_(T,V)$，$f_S=-T((partial S) / (partial l))_(T,V)$，则有
...
$ W(x,y,z)d x d y d z=(beta / sqrt(pi))^3 e^(-beta^2 (x^2+y^2+z^2))d x d y d z $
...
- 拉伸强度(strength elongation)：规定的试验温度，湿度，试验速度下，在标准试样上沿轴向施加拉伸载荷，直到试样被拉断为止。
...
- 拉伸模量（杨氏模量：拉伸初始阶段的应力与应变比例。且由于高聚物拉伸曲线非线性，只取初始阶段。）

$
  E= (Delta P slash b d) / (Delta l slash l_0)
$ <杨氏模量>

- 弯曲强度(bending strength)：规定试验条件下，对试样施加静弯曲力矩。
...
- 冲击强度(impact strength)：衡量材料韧性，表征抗冲击载荷破坏能力。
...
*拉伸模量*，或*杨氏模量*(Young's modulus)：

$
  E=sigma / epsilon = (F slash A_0) / (Delta l slash l_0)
$
...
其中$nu$为泊松比，定义为材料拉伸实验中材料横向宽度的减少与纵向长度的增加之比。大多数材料拉伸时，体积膨胀，$nu=0.2~0.5$，对于橡胶，$nu≈0.5$。

当理想不可压缩体拉伸变形时，体积不变，$(Delta V) / V_0=0$，$B=infinity$，$nu=0.5$，$E=3G$。

根据橡胶弹性统计理论，在小应变时，交联橡胶

$
  G = 1 / 3 E=(rho R T) / overline(M_c)
$
...
#figure(
  image("media/chapter3/分子量对弹性new.png", width: 350pt),
  caption: [不同重均分子量（$overline(M_w)$）和多分散度（$overline(M_w) slash overline(M_n)$）的市售聚苯乙烯的$log N_1$对$log sigma$图],
)
...
#figure(
  image("media/chapter3/爬杆.png", width: 250pt),
...
#figure(
  image("media/chapter3/入口压力降压力分布图.png", width: 350pt),
...
#figure(
  image("media/chapter3/加工因素温度.png", width: 250pt),
...
#figure(
  grid(
    ...
    gutter: 1em,
    align: center + horizon,
    image("media/chapter3/入口收敛流动.png", width: 150pt),
    image("media/chapter3/入口角与收敛角示意图.png", width: 180pt),
  ),
  caption: [入口收敛流动示意图],
)
...
- 都是可逆弹性应变的结果；
- 第一法向应力差、挤出胀大、出口压力都是聚合物熔体这种粘弹性流体弹性行为的表现形式。相互之间存在以下定量关系：
...
#figure(
  grid(
    ...
    gutter: 1em,
    align: center + horizon,
    image("media/chapter3/熔体破裂入口.png", width: 300pt), image("media/chapter3/压力降.png", width: 150pt),
  ),
...
#figure(
  grid(
    ...
    gutter: 1em,
    align: center + horizon,
    image("media/chapter3/HDPE剪切破裂.png", width: 150pt), image("media/chapter3/HDPE挤出形态.png", width: 250pt),
  ),
...
- 临界剪切应力 $τ_C$
  ...
  Benbow和Lamb以不同聚合物熔体出现不稳定流动时的剪切应力取其平均值，得到$tau_c=1.25 times 10^25 "N" slash "m"^2$ 。可认为流场中剪切应力超过此值时，熔体破裂发生。
...
- “弹性雷诺数”-韦森堡数（Wissenberg number）
  ...
  比较了弹性力与粘性力。当$"Nw"<1$时，液体为粘性流动，弹性形变很小；

  $"Nw"=1~7$时，液体为稳态粘弹性流体；$"Nw">7$时，液体为不稳定流动。
...
韦森堡数（Weissenberg Number）是表征弹性相对强度的核心无量纲数，当$"Nw"<1$，粘性主导，流动稳定；当$"Nw">7$时，弹性主导，液体为不稳定流动。增大韦森堡数可以促进聚合物分子链的取向。

==== 弹性行为的好处

由于拉伸硬化效应，有利于提高拉伸加工的稳定性；高弹性导致分子链取向，可以有效提高力学性能。

==== 弹性行为的弊端

如前所述，弹性行为会导致加工不稳定；影响制品表观质量，例如使表面粗糙等。

==== 削弱不利弹性行为的措施

从微观分子响应角度来说，应当尽量降低可逆应变；宏观弹性强度角度，要尽量降低韦森堡数。
...
1. 普弹形变：当高分子材料受到外力作用时，分子链内部键长和键角立刻发生变化，这种形变量是很小的，称为普弹形变，用$epsilon_1$表示
...
2. 高弹形变：分子链通过链段运动逐渐伸展的过程，形变量比普弹形变要大得多，但形变与时间成指数关系：

  $
    epsilon_2 = sigma / E_2(1-e^(-t slash tau))
  $ <高弹形变>

  $epsilon_2$：高弹形变

  $tau$：松弛时间，它与链段运动的粘度$eta_2$和高弹模量$E_2$有关，关系式为：

  $
    tau=eta_2 / E_2
  $

3. 粘性流动：分子间没有化学交联的线形高聚物，则还会产生分子间的相对滑移，形变为：

  $
    epsilon_3=sigma / eta_3 t
  $ <粘性流动>

  $eta_3$：本体形变。

普弹形变和高弹形变均为可逆形变，粘性流动为不可逆形变。高聚物受到外力作用时以上三种形变是一起发生的，材料的总形变为：

$
  epsilon(t)=epsilon_1+epsilon_2+epsilon_3=sigma / E_1+sigma / E_2(1-e^(-t slash tau))+sigma / eta_3 t
$ <总形变>

三种形变的相对比例依照具体条件不同而不同。

通常在高聚物的 $T_g$ 以上不远，可观察到较明显的蠕变现象。各种高分子材料蠕变情况相差很远。PTFE的蠕变相当严重，橡胶用硫化交联的方法防止蠕变。含芳杂环的高聚物具有良好的抗蠕变性。

*应力松弛*

应力松弛：在恒定温度和形变保持不变的情况下，高聚物内部的应力随时间增加而逐渐衰减的现象。例如拉伸一块未交联的橡胶到一定长度，并保持长度不变，随着时间的增长，这块橡胶的回弹力会逐渐减小，这是因为里面的应力在慢慢地减小，甚至可以减小到零。应力与时间也成指数关系：

$
  sigma=sigma_0 e^(-t slash tau)
$ <应力松弛>
...
关于动弹粘弹性更详细的内容见 3.3.4 动态粘弹性，此处仅介绍部分概念，便于引入力学模型。

=== 粘弹性的力学模型

==== Maxwell 模型

已知在高聚物发生蠕变时是由于高弹性和粘性引起的，可以通过弹性和粘性元件进行模拟。弹性元件（理想弹簧）符合胡克定律，$sigma=E epsilon=1 / D epsilon$，其中$E$和$D$分别为其模量和柔量。粘性元件（理想粘壶）表现牛顿粘性，符合$sigma=eta (d epsilon) / (d t)$，或$epsilon=sigma / eta t$。
...
由于$sigma=E epsilon_1$，故$d sigma slash d t=E d epsilon_1 slash d t$；又由$sigma = eta (d epsilon_2) / (d t)$，可得到Maxwell模型的运动方程：

$
  (d epsilon) / (d t) = 1 / E (d sigma) / (d t)+sigma / eta
$
...
应力松弛过程中，总形变一定，$d epsilon slash d t=0$。因此有：

$ (d sigma) / sigma = -E / eta d t $

当$t=0$时，$sigma=sigma_0$，上式积分得：
...
称为松弛时间，表示形变固定时由于粘性流动使应力减少到起始应力的$1 slash e$所需的时间。$t arrow.r infinity$时，应力松弛到0。
...
对模型施加一个交变应力$sigma(t)=sigma_0 e^(i omega t)$，由于$d sigma slash d t=i omega sigma$，可得：

#align(
  center,
  block[$(d epsilon) / (d t) = 1 / E (d sigma) / (d t)+sigma / eta=(1 / E+1 / (i omega eta))(d sigma) / (d t)$],
)

复数模量$ E^*=d sigma slash d epsilon=((d sigma) / (d t)) slash ((d epsilon) / (d t))=1 / (1 / E+1 / (i omega eta)) $代入$eta=tau E$，化简可得：

$
  E^*=(E omega^2 tau^2) / (1+ omega^2 tau^2)+i (E omega tau) / (1+ omega^2 tau^2)
$

其中，实数模量：

$
  E'=(E omega^2 tau^2) / (1+ omega^2 tau^2)
$

也称为储能模量，代表聚合物模量的弹性分量，反映应变作用下能量在熔体中的储存状况。

虚数模量：

$
  E''= (E omega tau) / (1+ omega^2 tau^2)
$

也称为损耗模量，为聚合物模量的黏性分量，反映材料在外加交变载荷作用下能量耗散的状况，二者的比值为损耗因子：
...
通过施加瞬时改变的应变（速率）或应力，来测量流体的响应随时间的变化。例如，应力发生阶跃变化（蠕变）或应变发生阶跃变化（应力松弛）。
...
*触变性*：流体或凝胶材料在一个恒定的剪切应力作用下，会发生剪切变稀，而当减小剪切应力后，其黏度会在一定时间内完全回复到初始状态，是一种时间依赖行为。
...
1. *旋转模式3ITT(低剪切-高剪切-低剪切,R-R-R模式)*
...
2. *振荡模式3ITT(小应变振荡-高剪切旋转-小应变振荡,O-R-O模式)*
  ...
  小应变振荡下可以得到$G'$和$G''$，可以同时表征材料的的黏弹行为

  高剪切旋转可以模拟材料高速涂布下的黏度性质。
  ...
  *应用-汽车喷漆涂*
  ...
  如图，在第三段测试中，刚开始涂料均为$G''>G'$，此时涂料还处于流动状态，当 $G'>G''$后，涂料变为固体状态，流平停止。

  故通过计算回复段 $G'=G''$对应的时间，可以对不同流变添加剂的抗流挂性能进行控制。
...
*瞬态测试*：通过施加瞬时改变的应变（速率）或应力，来测量流体的响应随时间的变化。
...
#summary-block(
  notion: [
    - 剪切粘度：剪切应力和剪切速率的比值，称为剪切粘度，也叫表观粘度，通常简称为粘度。
    - 熔融指数：在一定温度下，熔融状态的高聚物在一定负荷下，十分钟内从规定直径和长度的标准毛细管中流出的重量(克数)。
    - 毛细管流变仪剪切应力校正（Bagley校正，入口压力降校正）：压力传感器给出的压力值并不是毛细管中的压力降, 而是加上入口处压力降的结果。为了获得毛细管中的压力降，需要进行校正。校正思路是取几个不同L/D的毛细管，测定相同流量下的压力降，随后得到外推到0时的压力降，或者将入口压力降等效于某一长度，用经验公式进行矫正。
    - 毛细管流变仪剪切速率校正（Rabinowitsh-Mooney校正，非牛顿校正）：获得毛细管壁面处非牛顿流体的真实剪切速率。
    - 根据拉伸粘度随拉伸应变速率的变化不同，可将高分子液体分为三类：拉伸硬化、拉伸变稀、$eta_拉$不变。
    - 拉伸硬化原理：拉伸过程中，聚合物发生结晶，链段无法充分运动；或者聚合物发生缠结，物理交联，只能在链段间运动无法使整个分子链运动，导致链的活动性降低，粘度提高。
    - 触变剂的作用在于可以防止树脂在施工的斜面或垂直面上流淌，避免树脂含量在上下层不均匀现象，从而保证制品的质量。
    - 增稠剂是一种流变助剂，不仅可以使涂料增稠，防止施工中出现流挂现象，而且能赋予涂料优异的机械性能和贮存稳定性。
    - 高弹性是高聚物特有的基于链段运动的一种状态。这种状态是聚合物区别于其他材料的一个显著特征，也是聚合物展现优异性能的重要方面。
    - 当分子链被拉伸，高分子线团沿流动方向被拉伸，沿着与剪切力垂直的方向上被压缩，也即在垂直于剪切方向需要有力压住高分子线团，高分子线团对板有反推力。一旦压缩力撤除，线团有回复的趋势，熵增将驱动链构象回复至无规线团平衡态。此即聚合物弹性的微观本质。
    - 爬杆效应，也称Weissenberg效应，是高分子流体爬上旋转的搅拌棒或杆的现象，是流体粘弹性的一种表现形式。
    - 在管子进口与出口端出现与聚合物液体弹性行为有紧密联系的现象称之为端末效应，包括入口效应、出口膨胀效应。
    - 出口胀大效应，也称挤出胀大（extrudate swell），是指聚合物熔体在流动中产生高弹形变，在出口端发生高弹形变回复引起挤出物膨胀。
    - 高聚物熔体在剪切速率超过临界值时，会从稳定层流转变为不稳定流动，导致挤出物表面畸变。当熔体所受应力大于熔体自身的强度或者聚合物分子间作用力时，就会发生熔体破裂。这种不稳定性主要由弹性效应主导。
    - 在入口处，随着剪切速率上升，第二法向应力差会导致次级流动。即在这种径向压力梯度的驱动下，有向中心轴迁移趋势的熔体在入口区形成环状涡流。
    - 当高聚物熔体在管道中承受高剪切速率时，其与管壁界面的动态相互作用会引发滑-粘转变振荡。
    - 韦森堡数（Weissenberg Number）是表征弹性相对强度的核心无量纲数，当$"Nw"<1$，粘性主导，流动稳定；当$"Nw">7$时，弹性主导，液体为不稳定流动。增大韦森堡数可以促进聚合物分子链的取向。
    - 蠕变，指在一定的温度和较小的恒定外力(拉力、压力或扭力等)作用下，材料的形变随时间的增加而逐渐增大的现象。
    ...
    - 当应力的变化和形变的变化相一致时，没有滞后现象，每次形变所作的功等于恢复原状时取得的功，没有功的消耗。如果形变的变化落后于应力的变化，发生滞后现象，则每一循环变化中就要消耗功，称为力学损耗，有时也称为内耗。
    - 高分子材料的粘弹性对时间和温度具有等效性。对于同一力学松弛现象，既可在较高温度、较短时间内观察到，又可在较低温度、较长时间内观察到，即升高温度和延长观察时间对分子运动是等效的。
    - 作叠合曲线时，有时单靠水平移动叠加得不到光滑曲线，需要将不同温度得到的实验曲线垂直移动，这种手续称为垂直校正。
    - 动态力学松弛是在交变应力和应变作用下发生的滞后现象和力学损耗。
    - 粘弹性的本质: 不同的时空尺度（不同的时间或频率激发不同尺度的运动）导致不同的粘弹性表现。
    - 线性粘弹性：小应变；正比性：应力和应变或者应变速率成正比；加和性：不同时间应力的影响可以线性相加。
    - 非线性粘弹性：随着应变增加，模量随应变而变化。稳态高剪切速率下的显著弹性行为也是非线性粘弹性。
  ],
  equation: table(
    table.header(
...
Deborah 数打破了固体和流体响应的界限，提供了衡量粘弹性的定量尺子。De 越小，响应越快，性质更像液体。
...
#figure(image("media/chapter4/聚合物流动过程中的取向.png", width: 300pt), caption: [聚合物流动过程中的取向])

聚合物在加工过程中在管道型腔中的流动都是剪切流动，在剪切流动中速度梯度的作用下，卷曲状长链分子逐渐沿流动方向舒展伸直和取向。熔体温度高，分子热运动剧烈，因此在大分子流动取向的同时必然存在着解取向。

#figure(image("media/chapter4/流动过程中不同取向方式.png", width: 150pt), caption: [流动过程中不同取向方式])

流动取向可以是单轴或双轴的，取决于制件的结构形状、尺寸和熔体在其中的流动，截面积恒定，单轴取向，截面积变化，双轴取向或在更好的方向上取向。

流动对填料取向也有影响

#figure(image("media/chapter4/填料对取向的影响.png", width: 300pt), caption: [填料对取向的影响])
...
#figure(image("media/chapter4/剪切促进结晶微观机理.png", width: 300pt), caption: [剪切促进结晶微观机理])

剪切导致缠结网络取向，取向结构结晶形成串晶形貌

剪切和拉伸导致聚合物结晶成核速率增加。其机理包括：剪切取向使平衡熔点升高；外场作用下成核能垒下降。

#figure(image("media/chapter4/结晶成核速率与剪切速率关系.png", width: 300pt), caption: [结晶成核速率与剪切速率关系])
...
如图，对容易吹膜和不容易吹膜的材料都进行流变学测试验证假设，结果发现，容易吹膜的LDPE、LLDPE、长支化TH-PBAT入口压力降都比较高。而难吹膜的PHBHHx、线形PR-PBAT 入口压力降都比较低。
...
#figure(
  grid(
    ...
    gutter: 1em,
    align: center + horizon,
    image("media/chapter4/纺丝-5.png", width: 200pt), image("media/chapter4/纺丝-7.png", width: 150pt),
  ),
