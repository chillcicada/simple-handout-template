...
$
  lambda≈R^2 / D_R≈R^2 / (k T"/" N zeta)=zeta / (k T) N R^2∝N^2
$
...
#align(
  center,
  block[Utracki公式：$lg eta_m=omega_1 lg eta_1 + omega_2 lg eta_2$
  ],
)
...
$
  E= (Delta P"/" b d) / (Delta l "/" l_0)
$

- 弯曲强度(bending strength)：规定试验条件下，对试样施加静弯曲力矩。

$
  sigma_f = P / 2 (l_0"/"2) / (b d^2"/"6)
$
...
$
  E=sigma / epsilon = (F "/" A_0) / (Delta l "/" l_0)
$
...
根据橡胶弹性统计理论，在小应变时，交联橡胶$ G = 1 / 3 E=(rho R T) / overline(M_c) $
...
比较了弹性力与粘性力。当$N_w<1$时，液体为粘性流动，弹性形变很小；
...
韦森堡数（Weissenberg Number）是表征弹性相对强度的核心无量纲数，当$N_w<1$，粘性主导，流动稳定；当$N_w>7$时，弹性主导，液体为不稳定流动。增大韦森堡数可以促进聚合物分子链的取向。
...
$
  epsilon(t)=epsilon_1+epsilon_2+epsilon_3=sigma / E_1+sigma / E_2(1-e^(-t"/"tau))+sigma / eta_3 t
$
...
称为松弛时间，表示形变固定时由于粘性流动使应力减少到起始应力的$1"/"e$所需的时间。$t arrow.r infinity$时，应力松弛到0。
...
复数模量$ E^*=d sigma "/" d epsilon=((d sigma) / (d t))"/"((d epsilon) / (d t))=1 / (1 / E+1 / (i omega eta)) $代入$eta=tau E$，化简可得：
...
#align(
  center,
  block[$D^*=d epsilon"/"d sigma=((d epsilon) / (d t))"/"((d sigma) / (d t))=1 / E+1 / (i omega eta)=1 / E+1 / (i omega tau E)$],
)
...
$
  epsilon(t)=sigma_0 / E (1-e^(-t"/"tau))=epsilon(infinity)(1-e^(-t"/"tau))
$
...
#summary-block(
  ...
  equation: table(
    table.header([涵义], [公式], [索引]),
    [韦森堡数], [$ N_w = lambda dot(gamma) $ <->], [@eqt:韦森堡数],
  ),
  reference: [],
)
